...
impl IRPass for TypePass {
    ...
    fn run(&self, db: &mut DecompileDB) {
        ...
        {
            let mut reg_types: std::collections::HashMap<u64, std::collections::HashSet<XType>> = std::collections::HashMap::new();
            for &(reg, ref xtype) in prog.emit_var_type_candidate.iter() {
                reg_types.entry(reg).or_default().insert(xtype.clone());
            }
            let mut combo_counts: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
            for types in reg_types.values() {
                let mut sorted: Vec<_> = types.iter().map(|t| format!("{:?}", t)).collect();
                sorted.sort();
                let key = sorted.join(", ");
                *combo_counts.entry(key).or_insert(0) += 1;
            }
            let mut combos: Vec<_> = combo_counts.into_iter().collect();
